...
const Pictures = 'http://192.168.1.55:3000/Pictures/'
...
class MaintenanceScreen extends React.Component {
        ...
        return (
            <Modal animationType="slide" transparent={true}
                ...
                <Card>
                        ...
                        <Body>
                            <View style={{margin: 10}}>
                                ...
                                <View style={{
                                    ...
                                    <Thumbnail square large source={{uri: `${Pictures}${datos.imagen1}`}}/>
                                    {Espacio}
                                    <Thumbnail square large source={{uri: `${Pictures}${datos.imagen2}`}}/>
                                    {Espacio}
                                    <Thumbnail square large source={{uri: `${Pictures}${datos.imagen3}`}}/>
                                </View>
                                <View style={{flexDirection: 'row', margin: 3}}>
                                    <Thumbnail square large source={{uri: `${Pictures}${datos.imagen4}`}}/>
                                    {Espacio}
                                    <Thumbnail square large source={{uri: `${Pictures}${datos.imagen5}`}}/>
                                    {Espacio}
                                    <Thumbnail square large source={{uri: `${Pictures}${datos.imagen6}`}}/>
                                </View>
                    ...
                    <CardItem footer>
                        <View style={{left: 100}}>
                            <Button style={styles.centerButton} block rounded
                                    onPress={() => this.setModalVisible(!modalVisible)}>
                                <TxT
                                    style={{fontWeight: 'bold'}}>Cerrar &times;</TxT>
                            </Button>
                        </View>
                    </CardItem>
                </Card>
        ...
        if (loader) {
            return (
                <View style={{paddingVertical: 20, borderTopWidth: 1}}>
                    <Text style={{textAlign: 'center', fontWeight: 'bold'}}>Recargando...</Text>
                </View>
            )
        }
        if (error) {
    ...
    _renderItem = ({item, index}) => {
        return (
            <View style={style.CronoCard} key={index}>
                <Text style={style.CronoText}><Icon name={'clipboard-check'} size={15}/> Cliente: {item.ContactoCliente}
                </Text>
                <Text style={style.CronoText}><Icon name={'calendar-plus'} size={15}/> Fecha de
                    Ingreso: {item.fechaIngreso} - <Icon name={'clock'} size={15}/> {item.horaIngreso} </Text>
                <Text style={style.CronoText}><Icon name={'calendar-check'} size={15}/> Fecha de
                    Salida: {item.fechaSalida} - <Icon name={'clock'} size={15}/> {item.horaSalida}</Text>
                <TouchableOpacity style={style.ViewButton} onPress={() => this.setModalVisible(item)}>
    ...
    _ShowInfo = (item) => {
        ...
        ToastAndroid.showWithGravity(` Mantenimiento 🔧
        ...
        return (
            <Container>
                ...
                <Segment style={{backgroundColor: 'white'}}>
                    <Button active={this.state.activePage === 1 && color('white')}
                    ...
                    <Text>&nbsp;</Text>
                    <Button active={this.state.activePage === 2 && color('white')}
